...
class Cash extends WebLoginBase{
	...
	public final function inRecharge(){
		...
		if($para['amount']<=0) throw new Exception('儲值金额错误，请重新操作');
		if($id=$this->getValue("select bankId from {$this->prename}sysadmin_bank where id=?",$para['mBankId'])){
			if($id==2 || $id==3){
				if($para['amount']<$this->settings['rechargeMin1'] || $para['amount']>$this->settings['rechargeMax1']) throw new Exception('支付宝/财付通儲值最低'.$this->settings['rechargeMin1'].'元，最高'.$this->settings['rechargeMax1'].'元');
			}else{
				if($para['amount']<$this->settings['rechargeMin'] || $para['amount']>$this->settings['rechargeMax']) throw new Exception('银行卡儲值最低'.$this->settings['rechargeMin1'].'元，最高'.$this->settings['rechargeMax1'].'元');
			}
		}else{
				throw new Exception('儲值银行不存在，请重新选择');
			}
		...
		//}else{
			...
			if($this->insertRow($this->prename .'member_recharge', $para)){
				...
				throw new Exception('儲值订单生产请求出错');
			}
		}
		//清空验证码session
