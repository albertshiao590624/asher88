...
class box extends WebLoginBase{
	...
	public final function dowrite(){
		if(!$_POST) throw new Exception('提交資料出錯！');
		...
		if($touser=='parent'){
			...
			if(!$parentid=$this->getValue($sql,$this->user['uid'])) throw new Exception('您無上級代理！');
		    ...
		    foreach($arr as $key=>$var){
		         if($this->getValue($sql,$arr[$key])!=$this->user['uid']) throw new Exception('某些會員不是您的直屬下級！');
	...
	public final function answer($mid){
		...
		if($this->getValue($sql2,$mid)!=$this->user['uid']) throw new Exception('這不是您的消息,無法回復！');
	...
	public final function deleteAll($id){
		...
		$sql2="select to_uid from {$this->prename}message_receiver where mid=? and to_username='{$this->user['username']}'";
		foreach($arr as $key=>$var){
			 if($this->getValue($sql2,$arr[$key])!=$this->user['uid']) throw new Exception('這不是您的消息,無法刪除！');
    ...
    public final function detail($mid){
		...
		if($this->getValue($sql3,$mid)!=$this->user['uid']) throw new Exception('這不是您的消息！');
	...
	public final function senddetail($mid){
		...
		$sql2="select from_uid from {$this->prename}message_sender where mid=?";
		if($this->getValue($sql2,$mid)!=$this->user['uid']) throw new Exception('這不是您發送過的消息！');
