...
class Score extends WebLoginBase
{
	...
	{
		$_var_0 = 'select * from blast_yongjin where id>1 and type=1 order by endtime';
		$this->dataa = $this->getRows($_var_0);
		$_var_0 = 'select * from blast_yongjin where id=1';
	...
	public final function getyj()
	{
		...
		$_var_2 = "select todayget from blast_members where uid={$_var_1}";
		$_var_3 = $this->getRows($_var_2);
		if ($_var_3[0]['todayget'] == 1) {
			echo 'o';
			die;
		}
		$_var_2 = 'select * from blast_yongjin where id=1';
			...
			if ((int) $_var_4[0]['startime'] <= $_var_5) {
				if ($_var_5 <= (int) $_var_4[0]['endtime']) {
					if (!$this->noeasy()) {
						...
						$_var_6 = $this->noeasy();
						$_var_2 = "update blast_members set coin=coin+{$_var_6},todayget=1 where uid={$_var_1}";
						$this->exec($_var_2);
						$_var_2 = "select coin from blast_members where uid={$_var_1}";
						$_var_7 = $this->getRows($_var_2);
						$_var_8 = $_var_7[0]['coin'];
						$_var_9 = time();
						$_var_2 = "insert into blast_coin_log (actionTime,uid,liqType,coin,userCoin)values({$_var_9},{$_var_1},'56',{$_var_6},{$_var_8})";
						$this->exec($_var_2);
						$_var_10 = $this->user['username'];
						$_var_2 = "insert into blast_yj_log (uid,username,amount,actionTime)values({$_var_1},'{$_var_10}',{$_var_6},'{$_var_9}')";
						$this->exec($_var_2);
						echo 'y';
					}
					die;
	...
	public final function noeasy()
	{
		...
		$_var_12 = "select uid,parents from blast_members where parents like '%,{$_var_11},%' or parents like '{$_var_11},%'";
		...
		$_var_12 = "select uid,sum(mode*beiShu*actionNum) allmoney from blast_bets where uid in {$_var_15} and (actionTime between UNIX_TIMESTAMP('{$_var_20}')-24*3600 and UNIX_TIMESTAMP('{$_var_20}')) group by uid";
		...
		$_var_12 = 'select * from blast_yongjin where id>1 order by endtime desc';
	...
	{
		$_var_25 = 'select * from blast_hytzjlrule where money>0';
	...
	public final function fanxianvip()
	{
		...
		$_var_28 = "select vipjjtoday from blast_members where uid={$_var_26}";
		...
		$_var_28 = "select sum(mode*beiShu*actionNum) allmoney from blast_bets where uid={$_var_26} and actionTime > UNIX_TIMESTAMP('{$_var_30}') group by uid";
		$_var_31 = $this->getRow($_var_28);
		$_var_32 = $_var_31['allmoney'];
		$_var_28 = 'select * from blast_hytzjlrule where money>0 order by money desc';
		...
		$_var_28 = "update blast_members set coin=coin+{$_var_36},vipjjtoday=1 where uid={$_var_26}";
		$this->exec($_var_28);
		$_var_28 = "select coin from blast_members where uid={$_var_26}";
		$_var_38 = $this->getRows($_var_28);
		$_var_39 = $_var_38[0]['coin'];
		$_var_40 = time();
		$_var_28 = "insert into blast_coin_log (actionTime,uid,liqType,coin,userCoin)values({$_var_40},{$_var_26},'58',{$_var_36},{$_var_39})";
		$this->exec($_var_28);
		$_var_28 = "insert into blast_hytzjl_log (uid,username,amount,actionTime)values({$_var_26},'{$_var_27}',{$_var_36},'{$_var_40}')";
		$this->exec($_var_28);
		echo 'y';
		die;
	}
	...
	{
		if (!($_var_52 = intval($_var_52))) {
			throw new Exception('请求出错');
		}
		if (!($_var_53 = $this->getRow("select * from {$this->prename}score_swap where id={$_var_52}"))) {
			throw new Exception('请求出错');
		}
		if ($_var_53['uid'] != $this->user['uid']) {
			throw new Exception('你不能代别人取消兑换或确认收货');
		}
		if ($_var_53['state'] == 0) {
			throw new Exception('这兑换已经确认收货了');
		}
		if ($_var_53['state'] == 3) {
			throw new Exception('这兑换已经取消。');
		}
		if ($_var_53['state'] == 1) {
			$_var_54 = round($_var_53['score'] * $this->payout);
	...
	public final function dbqbed()
	{
		...
		$_var_78 = $this->getValue("select sum(beiShu * mode * actionNum) from {$this->prename}bets where actionTime > ? and uid={$this->user['uid']} and isDelete=0", $_var_75);
 
		if ($_var_78 < $this->dbqbsettings['xcoin']) {
	...
	//本金提款
	public final function dzyhtk_bj()
	{
		$getid = $_GET['id'];
	
		$_var_99 = "select id,ck_money,time,username from {$this->prename}dzyh_ck_swap where id={$getid}";
		...
		$this->display('score/dzyhtk_bj.php', 0, $_var_100);
	}
	
	//收益提款
	public final function dzyhtk_sy()
	{
		$getid = $_GET['id'];
		$_var_99 = "select id,ck_money,time,username,ljsy from {$this->prename}dzyh_ck_swap where uid={$this->user['uid']} and id={$getid}";
		if ($_var_100 = $this->getRow($_var_99)) {
			$_var_101 = ($this->time - $_var_100['time']) / 3600;
		} else {
			$_var_101 = 0;
		...
		$this->display('score/dzyhtk_sy.php', 0, $_var_100);
	}
	
	public final function dzyhcked()
	{
		
		//echo json_encode(array('code'=>0,'msg'=>'入金暂时关闭')); exit;
		
		$_var_104 = floatval($_POST['ckmoney']);
		
		if ($_var_104 <= 0) {

			echo json_encode(array('code'=>0,'msg'=>'输入的存款金额错误，请重新输入！')); exit;
		}
		if (md5($_POST['coinpassword']) != $this->user['coinPassword']) {
			echo json_encode(array('code'=>0,'msg'=>'资金密码错误,请核对后再操作')); exit;
		}
		if (strtolower($_POST['vcode']) != $_SESSION[$this->vcodeSessionName]) {
			echo json_encode(array('code'=>0,'msg'=>'验证码不正确。')); exit;
		}
		unset($_SESSION[$this->vcodeSessionName]);
		if (!$this->dzyhsettings['switchck']) {
			echo json_encode(array('code'=>0,'msg'=>'投资理财存款功能已关闭,详情请联系在线客服！')); exit;
		}
		...
		if ($this->user['coin'] == 0) {
			echo json_encode(array('code'=>0,'msg'=>'用户余额为零，请先儲值！')); exit;
		}
		if ($_var_104 > $this->user['coin']) {
			echo json_encode(array('code'=>0,'msg'=>'很遗憾！存款金额大于当前账户余额，无法存款，请先儲值！')); exit;
		}
		if ($_var_104 < 1000) {
			echo json_encode(array('code'=>0,'msg'=>'最低理财金额为1000元！')); exit;
		}
		if ($_var_104%1000 != 0) {
			echo json_encode(array('code'=>0,'msg'=>'理财金额必须为1000元的整倍数！')); exit;
		}
		
		if ($_var_104 < $this->dzyhsettings['tztj1di'] || $_var_104 > $this->dzyhsettings['tztj4gao']) {
			echo json_encode(array('code'=>0,'msg'=>'理财金额不正确！')); exit;
		}
		...
		if (!$this->insertRow($this->prename . 'dzyh_ck_swap', $_var_105)) {
			echo json_encode(array('code'=>0,'msg'=>'存款失败！请重试')); exit;
		}
		$this->addCoin(array('uid' => $this->user['uid'], 'coin' => -$_var_104, 'liqType' => 140, 'extfield0' => 0, 'extfield1' => 0, 'info' => '存入投资理财'));
		
		echo json_encode(array('code'=>1,'msg'=>'存款成功')); exit;
	}
	
	
	//取款方法-本金
	public final function dzyhtked_bj()
	{
		...
		if (!$this->dzyhsettings['switchtk']) {
			echo json_encode(array('code'=>0,'msg'=>'投资理财提款功能已关闭,详情请联系在线客服！')); exit;
		} 
		...
		if (!$_POST['yhkhh']) {
			echo json_encode(array('code'=>0,'msg'=>'请填写银行开户行')); exit;
		} 
		
		if (!$_POST['yhhm']) {
			echo json_encode(array('code'=>0,'msg'=>'请填写银行开户名')); exit;
		} 
		
		//当前存款
		$sql = "select * from {$this->prename}dzyh_ck_swap where id={$_POST['id']}";
		$user_money = $this->getRow($sql);
		/* if($user_money['state'] != 0){
			throw new Exception('对不起！您的存款已出局！');
		} */
		if($user_money['enable'] != 0){
			echo json_encode(array('code'=>0,'msg'=>'对不起！您的存款已被冻结,详情请联系在线客服！')); exit;
		}
		
		$sqlaaa = "select * from {$this->prename}dzyhtk where pid = {$_POST['id']} and info like '%本金%'";
		$user_moneys = $this->getRow($sqlaaa);
		if($user_moneys){
			echo json_encode(array('code'=>0,'msg'=>'您的本金已提款！')); exit;
		}
	
		//存款时间周期-后台设置的天数
		$zdts = $this->dzyhsettings['zdts'];
		$zdtsms = $zdts*24*60*60;
		
		//提款手续费
		$tksxf = $this->dzyhsettings['txsxf']/100;
		$kcmoney = $user_money['ck_money'] * $tksxf;
			
		//判断是否在这个天数内取款
		$moneyts = $user_money['time']+$zdtsms;
		
		
		if($this->time <= $moneyts){//有手续费
			$data = array('uid' => $this->user['uid'], 'username' => $user_money['username'], 'tk_money' => $user_money['ck_money'], 'time' => $user_money['time'], 'tktime' => $this->time, 'lv' => 0, 'lx' => 0, 'ip' => $this->ip(!0), 'isdelete' => 0, 'sxf' => $kcmoney ,'info' => '本金提款' , 'yhkh' => $_POST['yhkh'] , 'yhkhh' => $_POST['yhkhh'] , 'yhhm' => $_POST['yhhm'] , 'pid'=>$_POST['id']);
			...
			$datatk = array(
				...
				'lx'	=>	0,
				'sxf'	=>	$kcmoney,
				'money'	=> 	$user_money['ck_money'],
				'add_time'	=>	time(),
				'ck_time'	=> $user_money['time'],
				'status'	=> 0,
				'info'	=> '本金提款',
			);
		
			if (!$this->update("update {$this->prename}dzyh_ck_swap set state=1,ljsy=0 where id={$user_money['id']}")) {
				echo json_encode(array('code'=>0,'msg'=>'提款失败！请重试')); exit;
			}
			
		}else{//没有手续费
			
			$data = array('uid' => $this->user['uid'], 'username' => $user_money['username'], 'tk_money' => $user_money['ck_money'], 'time' => $user_money['time'], 'tktime' => $this->time, 'lv' => 0, 'lx' => 0, 'ip' => $this->ip(!0), 'isdelete' => 0, 'sxf' => 0,'info' => '本金提款到银行卡' , 'yhkh' => $_POST['yhkh'] , 'yhkhh' => $_POST['yhkhh'] , 'yhhm' => $_POST['yhhm'] , 'pid'=>$_POST['id']);
			...
			if (!$this->update("update {$this->prename}dzyh_ck_swap set state=1 where id={$user_money['id']}")) {
				echo json_encode(array('code'=>0,'msg'=>'提款失败！请重试')); exit;
			}
		...
		if (!$this->insertRow($this->prename . 'dzyh_tk_swap', $data)) {
			echo json_encode(array('code'=>0,'msg'=>'提款失败！请重试')); exit;
		}
		
		if (!$this->insertRow($this->prename . 'dzyhtk', $datatk)) {
			echo json_encode(array('code'=>0,'msg'=>'提款失败！请重试')); exit;
		}

		echo json_encode(array('code'=>1,'msg'=>'提款成功')); exit;
	}
	
	//取款方法-收益
	public final function dzyhtked_sy()
	{
		...
		//当前存款
		$sql = "select * from {$this->prename}dzyh_ck_swap where id={$getid}";
		$user_money = $this->getRow($sql);
		...
		$zmoney = $user_money['ljsy'];
		if($zmoney < 100){
			echo json_encode(array('code'=>0,'msg'=>'收益满100元才能提款！')); exit;
		}
	
		if($tkmoney > $zmoney){
			echo json_encode(array('code'=>0,'msg'=>'您的收益不足！请重新输入！')); exit;
		}
	
		$type = $_POST['type'];
		if($type == 0){
			echo json_encode(array('code'=>0,'msg'=>'请选择提款类型')); exit;
		}else if($type == 1){//银行卡提款
			if (!$_POST['yhkh']) {
				echo json_encode(array('code'=>0,'msg'=>'请填写银行卡号')); exit;
			} 
			...
			$data = array('uid' => $this->user['uid'], 'username' => $user_money['username'], 'tk_money' => $tkmoney, 'time' => $user_money['time'], 'tktime' => $this->time, 'lv' => 0, 'lx' => 0, 'ip' => $this->ip(!0), 'isdelete' => 0, 'sxf' => 0,'info' => '收益提款到银行卡' , 'status' => 0, 'yhkh' => $_POST['yhkh'] , 'yhkhh' => $_POST['yhkhh'] , 'yhhm' => $_POST['yhhm'], 'pid'=>$getid);
			...
			$data = array('uid' => $this->user['uid'], 'username' => $user_money['username'], 'tk_money' => $tkmoney, 'time' => $user_money['time'], 'tktime' => $this->time, 'lv' => 0, 'lx' => $tkmoney, 'ip' => $this->ip(!0), 'isdelete' => 0, 'sxf' => 0, 'status' => 1, 'info' => '收益提款到余额', 'yhkh' => '', 'yhkhh' => '' , 'yhhm' => '' , 'pid'=>$getid);
		...
		/* if (!$this->update("update {$this->prename}mrfengh set status=1 where pid={$getid}")) {
			echo json_encode(array('code'=>0,'msg'=>'提款失败！请重试')); exit;
		}
	
		if (!$this->update("update {$this->prename}mesxjfh set status=1 where did={$getid}")) {
			echo json_encode(array('code'=>0,'msg'=>'提款失败！请重试')); exit;
		} */
		...
		if (!$this->update("update {$this->prename}dzyh_ck_swap set ljsy=ljsy-{$tkmoney} where id={$getid}")) {
			echo json_encode(array('code'=>0,'msg'=>'提款失败！请重试')); exit;
		}
		
		$this->addCoin(array('uid' => $this->user['uid'], 'coin' => $tkmoney, 'liqType' => 150, 'extfield0' => 0, 'extfield1' => 0, 'info' => '投资理财提款'));
	...
	//分红收益明细
	public final function ffsy(){
	
		$this->display('score/ffsy.php');
	}
	
	//团队收益明细
	public final function tdsy(){
	
		$this->display('score/tdsy.php');
	}
	
	//提款明细
	public final function tkmx(){
	
		$this->display('score/tkmx.php');
	}
	
	//存款明细
	public final function ckmx(){
	
		$this->display('score/ckmx.php');
	}
}