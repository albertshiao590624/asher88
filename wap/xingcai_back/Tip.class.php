...
class Tip extends WebBase{
	...
	/**
	 *  儲值成功提示  //state=1 前台儲值  state=9 后台儲值
	 */
	public final function getCZTip(){
		...
		if($data=$this->getCol($sql)){
			...
			if($CZAmount>0){
				return array(
					'flag'=>true,
					'message'=>'儲值成功！<br>儲值金额：'.$CZAmount.'元'
				);
			}else{
				return array(
					'flag'=>true,
	...
	public final function getYKTip($type, $ctionNo){
	 ...
	 if($type && $ctionNo){
		...
		if($this->getCol("select id from {$this->prename}bets {$whereStr}")){
			...
			if($ykMoney>0){
				$messager=$czName." 第".$ctionNo."期：<br />盈亏 <font style='color:#F00;font-weight:bold;font-size:14px;'>".round($ykMoney,3)."</font> 元";
			
			}else{
				$messager=$czName." 第".$ctionNo."期：<br />盈亏 <font style='color:#060;font-weight:bold;font-size:14px;'>".round($ykMoney,3)."</font> 元";
