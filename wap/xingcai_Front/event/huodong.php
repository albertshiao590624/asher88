...
?>
		<p class="counyongjin">今日已儲值<span id="days"><?=$this->iff($rechargeAmount,$rechargeAmount,0)?></span>元</p>
		<?php
		if(!$ret){ ?>
        	<a href="#" onclick="hdSign(<?=$item['id']?>,<?=$this->user['uid']?>,'chose_<?=$item["id"]?>')" class="check_btn">领取儲值任务</a>
		<?php }else{?>
			<a href="#" onclick="hdGet(<?=$item['id']?>,<?=$this->user['uid']?>,'chose_<?=$item["id"]?>')" class="check_btn">领取儲值福利</a>
		<?php } ?>
		    </div>
    <div class="activity_nr">
        ...
        <div class="x-text">
			...
			<?php
                ...
                $tplMain = <<<TPL
				每日首次儲值 %s 元以上（&nbsp;最高可以领取%s）<p></p> 
                %s
TPL;
                $tplTip = <<<TIP
				%s 活动%s&nbsp;&nbsp;&nbsp;(&nbsp;%s 倍流水可领取首次儲值金额 %s ％的活动礼金&nbsp;)<p>&nbsp; </p> 
...
TIP;
      ...
      <h4 class="x-title">注意事项</h4>
        <div class="x-text"><p><p> </p> <p> </p> <p>1、当天首次儲值满足活动要求，且当天消费达到流水要求即可领取对应的活动奖金。</p> <p>2、用户打开官网活动公告中的“热门活动”，点击“儲值福利”页面上的“领取儲值福利”即可完成奖金领取。</p> <p>3、奖金需在当天(23:59:59之前)完成领取，隔天无效。</p> <p>4、用户自身领取奖金后出款，要求满足对应奖金的流水要求。</p> <p>5、本次活动计算彩票投注，可与其他活动共享。</p> <p>6、同一账户、同一银行卡、同一IP每天只可参加一次活动。</p> <p>7、任何的无风险投注，对冲，对打均不计算有效投注。有效投注定义：玩法投注不得超过该玩法投注最大注数的70%，即定位胆不得超过7注，二星不得超过70注，三星不得超过700注，四星不得超过7000注，五星不得超过70000注。如发现利用活动存在对冲等恶意套利行为，阿舍仔娛樂城有权扣除违规所得。</p> <p>8、【阿舍仔娛樂城】有权对活动随时做出更改、终止，并享有最终解释权。</p> <p> </p> <p> </p> <p> </p> <p> </p> <p> </p> <p> </p> <p> </p> <p> </p> <p> </p> <p> </p> <p> </p> <p> </p></p></div>
    </div>
	<?php } ?>
        <?php } ?>
