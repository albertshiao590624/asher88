<?php
	...
	if($para['parentId']=intval($para['parentId'])){
		...
		$uid=$uParentId;
	}elseif($para['username'] && $para['username']!='帳號名'){
		// 帳號名限制
...
<div class="row">
	...
	?>
            ...
            <td>
                <?php if(!$noChildren){?>
                <a target="ajax" dataType="html" call="searchData" class="qzbtn" href="<?="{$rel}/?parentId={$var['uid']}&fromTime={$para['fromTime']}&toTime={$para['toTime']}"?>">下級</a>
				<?php }?>
                <?php if($var['uid']!=$this->user['uid']&&$var['parentId']){?>
                  <a target="ajax" dataType="html" call="searchData" class="qzbtn" href="<?="{$rel}/?uid={$var['uid']} &fromTime={$para['fromTime']}&toTime={$para['toTime']}"?>">上級</a>
				<?php }?>
            </td>
		</tr>
	<?php } ?>
	
	<?php if($para['userType']==1 || ($para['userType']==0 && !$para['parentId']) ||($para['username'] && $para['username']!='帳號名')){//------------------------------------------
		...
		<tr>
			<td><span class="spn9">團隊總結</span></td>
		...
		<tr>
			<td><span class="spn9"><?php if(intval($para['userType'])==2){echo '直接下級';}else if(intval($para['userType'])==3){echo '所有下級';}else{echo '直接下級';}?></span></td>
            <!--td><?=$this->ifs($all['rechargeAmount'], '--')?></td>
			<td><?=$this->ifs($all['cashAmount'], '--')?></td-->
			<td><?=$this->ifs($all['betAmount'], '--')?></td>
			<td><?=$this->ifs($all['zjAmount'], '--')?></td>
			...
			?>
		<tr>
			<td><span class="spn9">所有下級</span></td>
            <!--td><?=$this->ifs($all['rechargeAmount'], '--')?></td>
			<td><?=$this->ifs($all['cashAmount'], '--')?></td-->
			<td><?=$this->ifs($all['betAmount'], '--')?></td>
			<td><?=$this->ifs($all['zjAmount'], '--')?></td>
