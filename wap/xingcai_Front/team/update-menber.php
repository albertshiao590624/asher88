	...
	<table cellpadding="2" cellspacing="2" class="form-tips" style="width:100%;margin-top: 5px;">
      <?php if($userData['parentId']){ ?>
		...
		<tr>
			...
			<td><input type="text" name="fanDian" style="width:100px; height:22px; vertical-align:middle;" value="<?=$userData['fanDian']?>" max="<?=$parentData['fanDian']?>" min="0" fanDianDiff=<?=$this->settings['fanDianDiff']?> val="<?=$userData['fanDian']?>" >%&nbsp;&nbsp;<span style="color:#999">0～<?=$this->iff($parentData['fanDian']-$this->settings['fanDianDiff']<=0,0,$parentData['fanDian']-$this->settings['fanDianDiff'])?>%</span></td>
		</tr>
        <tr>
        	<td class="title">最後登錄：</td>
