...
<form action="/index.php/team/linkUpdateed" target="ajax" method="post" call="linkDataSubmitCode" onajax="linkDataBeforeSubmitCode" dataType="html">
	...
	<tr>
			...
			<td><label><input style="width: auto;" type="radio"  name="type" value="1" title="代理" <?=$this->iff($linkData['type'],'checked="checked"','')?> />代理</label>
			<label><input style="width: auto;" name="type" type="radio" value="0" title="會員" <?=$this->iff(!$linkData['type'],'checked="checked"','');?>/>會員</label></td>
	</tr>
	<tr>
			<td class="title">返點：</td>
			<td><input type="text" class="input150" name="fanDian" value="<?=$linkData['fanDian']?>" max="<?=$parentData['fanDian']?>" min="0" fanDianDiff=<?=$this->settings['fanDianDiff']?> >% <span style="color:#999">(0~<?=$parentData['fanDian']?>%)</span></td>
	</tr>
    <tr>
