...
<ul class="list_menus-li">
	<li class="on"><a href="/index.php/team/addlink">推廣設定</a></li>
	<li><a href="/index.php/team/linkList">連結管理</a></li>
</ul>
</div>
...
<div class="form_switch_head switch_head">
	<div class="form_item1 clearfix">
		<span class="item_left">溫馨提示：</span>
		<div class="item_right entry_two">
		<p>★推廣連結註冊使用者返點最大為<?=$this->iff(($this->user['fanDian']-$this->settings['fanDianDiff'])<=0,0,$this->user['fanDian']-$this->settings['fanDianDiff'])?>%</p>
	
		</div>
	</div>
	<div class="form_item clearfix">
		<span class="item_left">帳號類型: </span>
		<div class="item_right">
			<div class="switch_choose">
				...
				<label class="bk_r" for="utype2">會員</label>
						</div>
		</div>
	</div>
	<div class="form_item clearfix">
		<span class="item_left">返點設置: </span>
		<div class="item_right entry_one">
		<input type="text" name="fanDian" max="<?=$this->user['fanDian']?>" max="<?=$this->user['fanDian']?>"  fanDianDiff=<?=$this->settings['fanDianDiff']?> maxlength="5" value="0.1" placeholder="請設置返點">
		<span id="rebate_fp" class="rebate_fp1">返點區間(0%-<?=$this->iff(($this->user['fanDian']-$this->settings['fanDianDiff'])<=0,0,$this->user['fanDian']-$this->settings['fanDianDiff'])?>%)</span>
		</div>
	</div>
	<!--div class="form_item clearfix" style="padding-top:0.5rem">
		<div class="item_right" style="float:right">
			<span class="rebate_zs">您的下級返點為: <b id="subPoint">4.9%</b></span>
			<span class="rebate_zs">您的返點：<b>5.0%</b></span>
		</div>
	</div-->
	<div class="form_submit_box">
		<input class="button" id="addlink" type="submit" value="生成連結">
	</div>
</div>
</form>
