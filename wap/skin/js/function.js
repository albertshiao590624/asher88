...
function userCoinBeforeSubmitCode(){
	if(this.coin.value<=0) throw('金額必須大於0');
}
...
function newcode(code_arr){
	var _html='';
	if(game.type==20||game.type==65||game.type==66){ //pk10開獎動畫
		...
		$("#num").empty().html(_html).css({'width': 95+code_arr.length + "%"});//時時彩開獎動畫位置	
		$("#num li").each(function(index){
...
function codeplay(code_arr){
	var _html='';
	if(game.type==20|| game.type==61|| game.type==73|| game.type==74|| game.type==71|| game.type==72||game.type==78|| game.type==65||game.type==66){//正在開獎中彩球大小
		if(code_arr.length==20||code_arr.length==65||code_arr.length==66){
			...
			$("#num").empty().html(_html).css({'width': 95+code_arr.length + "%"});//PK10 快樂十分彩球位置
			$("#num li").each(function(index){
				var num = $(this);
				num.animate({ 
					top:"50px"
		...
		if(code_arr.length==7){
			...
			$("#num").addClass("lhc").empty().html(_html).css({'width': 44*code_arr.length + "px"});//正在開獎彩球位置
		}else{
			$.each(code_arr,function(index,value){
				_html += "<li><span>-</span></li>";
			});
			$("#num").removeClass("lhc").empty().html(_html).css({'width': 95+code_arr.length + "%"});//正在開獎彩球位置
		}
		$("#num li").each(function(index){
			var num = $(this);
		...
		$("#num").empty().html(_html).css({'width': 95+code_arr.length + "%"});//重慶彩開獎位置
		$("#num li").each(function(index){
			var num = $(this);
			num.animate({ 
				top:"50px"
...
function gameKanJiangDataC(diffTime, actionNo){
	...
	if(diffTime<=0){
		...
		if(S){
			xingcai('當期銷售已截止，請進入下一期購買。');
			$(".layui-m-layerbtn span").html("(5) 確定");
			
			var second = 4;
			timer5 = window.setTimeout(timeFun5,1000);
			
			function timeFun5(){
				$(".layui-m-layerbtn span").html("(" + second + ") 確定");
				second--;
				if(second < 0){
...
function setKjing(){
	...
	if(!KS){
		$('#kaijiang #kjsay').html('<em class="kjtips">正在開獎中</em>');
		$('#kaijiang #kjsay').show();
		$('.wjkjData p').hide();
		$('.wjkjData ul').show();
	}
...
function safeBeforSetPwd(){
	if(!this.oldpassword.value){xingcai("請輸入舊登入密碼");return false;}
	if(this.oldpassword.value.length<6){xingcai("舊登入密碼至少6位元");return false;}
	if(!this.newpassword.value){xingcai("請輸入新登入密碼");return false;}
	if(this.newpassword.value.length<6){xingcai("新登入密碼至少6位元");return false;}
	if(!this.qrpassword.value){xingcai("請確認新登入密碼");return false;}
	var confirmpwd=$(':password.confirm', this).val();
	if(confirmpwd!=this.newpassword.value){xingcai("兩次輸入密碼不一樣");return false;}
	return true;
}
/**
 * 設置資金密碼前調用
...
function safeBeforSetCoinPwd(){	
	if(!this.newpassword.value){xingcai("提款密碼不符合規則，請重新輸入");return false;}
	if(this.newpassword.value.length<6){xingcai("提款密碼至少6位元數");return false;}	
	var confirmpwd=$(':password.confirm', this).val();
	//if(confirmpwd!=this.newpassword.value){xingcai("兩次輸入密碼不一樣");return false;}	
	return true;
}
...
function safeBeforSetCoinPwd2(){	
	if(!this.oldpassword.value){xingcai("請輸入舊提款密碼");return false;}
	if(this.oldpassword.value.length<6){xingcai("舊提款密碼至少6位元");return false;}
	if(!this.newpassword.value){xingcai("請輸入新提款密碼");return false;}
	if(this.newpassword.value.length<6){xingcai("新提款密碼至少6位元");return false;}
	if(!this.qrpassword.value){xingcai("請確認新提款密碼");return false;}
	var confirmpwd=$(':password.confirm', this).val();
	if(confirmpwd!=this.newpassword.value){xingcai("兩次輸入的新提款密碼不一樣");return false;}
	return true;
}
...
function safeBeforSetCBA(){
	if(!this.bankId.value){xingcai("請選擇銀行類型");return false;}
	if(!this.countname.value){xingcai("支行地址沒有填寫");return false;}
	if(!this.username.value){xingcai("開戶姓名沒有填寫");return false;}
	if(!this.account.value){xingcai("銀行卡號沒有填寫");return false;}
	if(this.account.value.length<16){xingcai("銀行卡號至少16位數");return false;}
	if(!this.coinPassword.value){xingcai("請輸入提款密碼");return false;}
	if(this.coinPassword.value.length<6){xingcai("提款密碼至少6位元數");return false;}
	return true;
}
/**
 * 修改銀行資訊調用
...
function teamBeforeAddMember(){
	...
	if(!this.username.value){xingcai("請輸入會員帳號");return false;}
	if(!/^\w{5,16}$/.test(this.username.value)){xingcai("帳號名由5到16位元的字母或數位組成");return false;}
	if(!this.password.value){xingcai("請輸入登入密碼");return false;}
	if(this.password.value.length<6){xingcai("登入密碼至少6位元數");return false;}
	if(!this.cpasswd.value){xingcai("請確認登入密碼");return false;}
	if(document.getElementById('cpasswd').value!=this.password.value){xingcai("兩次輸入的登入密碼不一樣");return false;}
	if(!this.qq.value){xingcai("請輸入QQ帳號");return false;}
	if(this.qq.value.length<5){xingcai("QQ帳號至少5位數");return false;}
	if(!this.fanDian.value){xingcai("請輸入返點");return false;}
	if(parseFloat(this.fanDian.value)<0){xingcai("返點不能小於0%"); return false;}
	if(parseFloat(this.fanDian.value)>parseFloat($(this.fanDian).attr('max'))){xingcai('返點不能大於'+$(this.fanDian).attr('max')); return false;}
	var fanDianDiff= $(this.fanDian).attr('fanDianDiff');
	if((this.fanDian.value*1000) % (fanDianDiff*1000)){xingcai('返點只能是'+fanDianDiff+'%的倍數');return false;}
}
//新增會員
...
function userDataBeforeSubmitCode(){
	...
	if(!this.fanDian.value.match(/^[\d\.\%]{1,4}$/)) throw('請正確設置返點');
	if(parseFloat(this.fanDian.value)>=parseFloat($(this.fanDian).attr('max'))) throw('返點不能大於或等於'+$(this.fanDian).attr('max'));
	if(parseFloat(this.fanDian.value)<parseFloat($(this.fanDian).attr('min'))) throw('返點不能小於'+$(this.fanDian).attr('min'));
	if(parseFloat(this.fanDian.value)<parseFloat($(this.fanDian).attr('val'))) throw('返點不能小於'+$(this.fanDian).attr('val'));
	var fanDianDiff= $(this.fanDian).attr('fanDianDiff');
	if((this.fanDian.value*1000) % (fanDianDiff*1000)) throw('返點只能是'+fanDianDiff+'%的倍數');
}
...
//存款入金
function userck(err,data){
	data = JSON.parse(data);

	if(data.code == 0){
		xingcai(data.msg,"err");
	}else{
		xingcai(data.msg,"ok");
		$(this).parent().dialog('destroy');
		reload();
...
function xzyhk(){
	var ids = $('.myselect option:selected').val();
	if(ids == 1){
		$('.yhkh').show();
		$('.yhkhh').show();
		$('.yhhm').show();
...
function teamBeforeAddLink(){
	var type=$('[name=type]:checked',this).val();
	if(!this.fanDian.value.match(/^[\d\.\%]{1,4}$/)) throw('請正確設置返點');
	if(!this.fanDian.value) throw('請輸入返點');
	if(parseFloat(this.fanDian.value)<0) throw('返點不能小於0%');
	if(parseFloat(this.fanDian.value)>parseFloat($(this.fanDian).attr('max'))) throw('返點不能大於'+$(this.fanDian).attr('max'));
	var fanDianDiff= $(this.fanDian).attr('fanDianDiff');
	if((this.fanDian.value*1000) % (fanDianDiff*1000)) throw('返點只能是'+fanDianDiff+'%的倍數');
}
//新增註冊連結
...
function reloadMemberInfo() {
                $.ajax({
                    ...
                    success : function(data){
                        autoRefresh = true;
                        if( data == "error" ) {//
                            $("#refff").html("<b>正在讀取資金</b>");
                            return false;
                        } else {
                            if(isNaN(data)){
                                layer.open({
                                    content:"獲取餘額失敗，您的登錄可能已經過期，請重新登錄",
                                    btn:'確定',
                                    yes:function(index){
                                        location.href="/";
                                        layer.close(index)
...
function setGameZhuiHao(data){
	...
	$.get('/index.php/index/zhuiHaoModal', function(html){
		$(html).dialog({
			title:'追號期數：<span class="qs">0</span>　總金額：<span class="amount">0.00</span>元',
...
function gameMsgAutoTip(){
	...
	if(calcFun && (calcFun=window[calcFun]) && (typeof calcFun=='function')){
		try{
			...
			$('#game-tip-dom').text('共'+obj.actionNum+'注，金額'+(gameGetMode()*gameGetBeiShu()*obj.actionNum).round(3)+'元');
		}catch(err){
 ...
 * 清除號碼
 *
 * @params bool isSelected	是否只清除選中的項，默認false
 */
...
function gameAddCode(code){
	...
	if(code.actionNum==0) throw('號碼不正確');
	try{
		...
		if((weizhiTypejsh=='z3_r3_zuhetouzhu'&&caizhongTypejsh=='22')
			...
			var weiShu=0;
			if($(':checked', $wei).length<$wei.attr('length')) throw('至少需要選擇'+codeLen+'個位置');
			...
			for(var iii=0;iii<weizhiAr.length;iii++){
				...
				delete code.isZ6;
				if($wei.length){
					//if($(':checked', $wei).length!=weiCount) throw('請選擇'+weiCount+'位數！');
					//TODO:0507--jinshanhu
				...
				.append(
					$('<td>').append('獎－返：'+parseFloat(code.bonusProp).round(3)+'-'+parseFloat(code.fanDian).round(1)+'%')
				)
				.append(
			...
			if($wei.length){
				...
				if((weizhiTypejsh=='2x_r2d_zuhetouzhu'&&caizhongTypejsh=='30')
					...
					||(weizhiTypejsh=='z3_r6_zuhetouzhu'&&caizhongTypejsh=='23')
					||(weizhiTypejsh=='3x_rz3_zuhetouzhu'&&$('#wei-shu').attr('playedIdjsh')=='24')){
					if($(':checked', $wei).length<weiCount) throw('請選擇'+weiCount+'位數(1)！');
				}else{
					//if($(':checked', $wei).length!=weiCount) throw('請選擇'+weiCount+'位數(2)！');
				}
				$(':checked', $wei).each(function(){
					weiShu|=parseInt(this.value);
				});
			...
			//.append(
			//	$('<td>').append('獎－返：'+parseFloat(code.bonusProp).round(2)+'-'+parseFloat(code.fanDian).round(1)+'%')
			//)
			.append(
				$('<td><div class="del"></div></td>')
			)
...
function gamePostCode(){
	...
	var tipString='';
	tipString+='<br /><table width="100%" class="form-tips"><tr><th>注數</th><th>倍數</th><th>模式</th></tr>';
	$('.touzhu-cont tr').each(function(){
		var $this=$(this);
...
function gameActionAddCode(){
	//獎金返點限制[如獎金模式在1920以下才能購買分模式(返點大於最大返點-11)]
	//代理不能買單
	if($('#wjdl'))
	{
	...
	if(userFanDian-fanDian> modeFanDian){
		...
		xingcai(modeName[mode]+'模式最大獎金只能為'+_amount.toFixed(3));
		return false;
	}
	// 單筆中獎限額
	var maxZjAmount=$slider.data().betZjAmount;
		//console.log('限額：%s, 中獎：%s', maxZjAmount, gameGetPl() * mode * ($('#beishu').val()||1));
	if(maxZjAmount){
		if($('#fandian-value').data('pl') * mode/2 * ($('#beishu').val()||1) > maxZjAmount){
			xingcai('單筆中獎獎金不能超過'+maxZjAmount+'元');
			return false;
		}
	}
	var obj,$game=$('#num-select .pp'),
	...
	if(calcFun && (calcFun=window[calcFun]) && (typeof calcFun=='function')){
		try{
			...
			if(maxBetCount && obj.actionNum>maxBetCount){
				xingcai('單筆投注注數最大不能超過'+maxBetCount+'注');
				return false;
			}
...
function confirmCancel() {  
        //利用對話方塊返回的值 （return true 或者return false）  
        if (confirm("是否確定撤單？")) {  
            xingcai("撤單成功");  
			return true;
        }  
 ...
 * 設置反點
 *
 * @params value		反點值，可以是個浮點數，表示在當前值時的增量
 */
...
function gameGetBeiShu(){
	...
	if(isNaN(txt=parseInt(txt))) throw('倍數設置不正確');
	return txt;
}
//{{{ 相關演算法集
...
function tzAllSelect(){
	...
	if(this.has('.checked').length!=codeLen) throw('請選'+codeLen+'位元數字');
...
function tz5xBDweix(){
    ...
    if($('#wei-shu :checked',this).length<3) throw('請選3個及以上位置');
...
function tzSscInput(){
	...
	if(str.length && str.length % codeLen == 0){
		if(/[^\d]/.test(str)) throw('投注有錯，不能有數位以外的字元。');
...
function ssc2xzxds(){
	...
	//if(codes2.toString()!=codes.toString()) xingcai("系統已自動過濾重複號碼");
...
function tz11x5Inputrxds(){
	...
	if(isRepeat(str2)) throw('號碼有重複，請重新輸入!');
	if(str.length<codeLen) throw('至少輸入'+parseInt(this.attr('length'))+'個號!');
	if(str.length && str.length / codeLen == 1){
		if(/[^\d]/.test(str)) throw('投注有錯，不能有數位以外的字元!');
		for(var j=0;j<str2.length;j++){
			if(info.indexOf(str2[j])==-1) throw('號碼輸入有誤，請重新輸入!');
		}
		codes=codes.concat(str.match(new RegExp('\\d{'+codeLen+'}', 'g')));
	}else{
		len=0;
...
function tzSscZuInput(){
	...
	codes.forEach(function(code){
		//if((new RegExp("^(\\d)\\1{"+(codeLen-1)+"}$")).test(code)) throw('組選不能為豹子');
	});
	codes=codes.map(function(code){
		return code.split("").join(',')
	});
...
function tzSscWeiInput(){
	...
	if((weizhiTypejsh=='2x_r2d_zuhetouzhu'&&caizhongTypejsh=='30')
			||(weizhiTypejsh=='2x_r3d_zuhetouzhu'&&caizhongTypejsh=='15')
			||(weizhiTypejsh=='dx_r4d_zuhetouzhu'&&caizhongTypejsh=='9')){
		if($('#wei-shu :checked',this).length<codeLen) throw('請選'+codeLen+'位數~');
	...
	//if(reTemp.length>0) xingcai('存在輸入重複的資料 '+reTemp.join(','));
...
function tzSscZuWeiInput(){
	...
	var caizhongTypejsh=$('input[name="playedId"]').val();
		...
		if($('#wei-shu :checked',this).length<codeLen) throw('請選'+codeLen+'位數(1)');
		$('#wei-shu :checkbox',this).each(function(i){
			if(this.checked) weiShu.push(i);
			//alert(i);
		});
...
function tzCombineSelect(){
	...
	var caizhongTypejsh=$('input[name="playedId"]').val();
	    ...
	    if($select.length!=codeLen) throw('請選'+codeLen+'位數(1)');
		if($('#wei-shu :checked',this).length<codeLen) throw('請選'+codeLen+'位數(2)');
		$('#wei-shu :checkbox',this).each(function(i){
			if(this.checked) weiShu.push(i);
		});
...
function tzCombineSelect_1(){
	...
	if($select.length<codeLen) throw('請選'+codeLen+'位數');
	$select.each(function(){
		codes+=this.value;
	});
...
function tzCombineSelect_1x(){
	...
	if($('#wei-shu :checked',this).length<codeLen) throw('請選'+codeLen+'個及以上位置');
...
function tzSscHhzxInput_1(){
	...
	z=[],
	n=0;
	var o={"前":[16,17],"中":[289,290],"後":[19,20],"任選":[22,23],"混":[59,60]};
	if(played=='任選' && $('#wei-shu :checked',this).length <3) throw('請選3位及以上的數');
	codeList=codeList.replace(/[^\d]/gm,'');
	if(codeList.length==0) throw('請輸入號碼');
	if(codeList.length % 3) throw('輸入號碼不正確');
	...
	if(z3.length && z6.length){
		return [{playedId:o[played][0], playedName:played+'三組三', actionData:z3.join(','), actionNum:z3.length, isZ6:false},
				{playedId:o[played][1], playedName:played+'三組六', actionData:z6.join(','), actionNum:z6.length, isZ6:true}];
	}else if(z3.length){
		return {playedId:o[played][0], playedName:played+'三組三', actionData:z3.join(','), actionNum:z3.length, isZ6:false};
	}else if(z6.length){
		return {playedId:o[played][1], playedName:played+'三組六', actionData:z6.join(','), actionNum:z6.length, isZ6:true};
	}
}
function ssc_z3_r6(){
	var codeLen=parseInt(this.attr('length')),
...
function tzSscHhzxInput_2(){
	...
	if(weizhiTypejsh=='3x_rz3_zuhetouzhu'&&$('#wei-shu').attr('playedIdjsh')=='24'){
		if(played=='任選' && $('#wei-shu :checked',this).length<3) throw('請選3位數');
	}else{
		if(played=='任選' && $('#wei-shu :checked',this).length!=3) throw('請選3位數');
	}
	codeList=codeList.replace(/[^\d]/gm,'');
...
function tz11x5Select(){
	...
	if(sType){
		...
		if(code.length<codeLen) throw('至少選擇'+codeLen+'位數');
