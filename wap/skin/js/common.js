...
layer.open({
  ...
  shade: 0.8,
  area: ['700px', '600px'],
  content: 'http://wpa.qq.com/msgrd?v=3&uin=3390808167&site=qq&menu=yes'
})
}
...
/*  财务中心-儲值界面-复制账户名以及卡号等资料的弹窗以及复制功能 */
...
/*  财务中心-儲值界面-确定按钮执行在框架内 */
function showPaymentFee() {
   $("#ContentPlaceHolder1_txtMoney").val($("#ContentPlaceHolder1_txtMoney").val().replace(/\D+/g, ''));
   jQuery("#chineseMoney").html(changeMoneyToChinese($("#ContentPlaceHolder1_txtMoney").val()));
        }
/*  财务中心-儲值界面-儲值框内填写金额判定 */
function checkRecharge(){
	if(!this.amount.value) throw('请填写儲值金额');
	//showPaymentFee();
	//if(isNaN(amount)) throw('儲值金额错误');
	//if(!this.amount.value.match(/^\d+(\.\d{0,2})?$/)) throw('儲值金额错误');
	showPaymentFee();
	...
	if($('input[name=mBankId]').val()==2||$('input[name=mBankId]').val()==3){
		if(amount<min1) throw('支付宝/财付通儲值金额最小为'+min1+'元');
		if(amount>max1) throw('支付宝/财付通儲值金额最多限额为'+max1+'元');
		showPaymentFee();
	}else{
		if(amount<min) throw('儲值金额最小为'+min+'元');
		if(amount>max) throw('儲值金额最多限额为'+max+'元');
		showPaymentFee();
