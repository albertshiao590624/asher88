...
function newcode(code_arr){
	...
	}else if(game.type==34 || game.type==35){
		code_arr.splice(6,0,'-');
		$.each(code_arr,function(index,value){
...
function codeplay(code_arr){
	...
	/*用于处理奖球的初始化*/
	function fillBall(){
		var _node = $("#num"),_nodesHtml = _node.html(),_key,_type,_html = '',i = 0;
		var lottery_type = {
			...
			'hnquick5' 	: { 'len' : 5, 'num' : '-', 'width' : 50 },//腾讯5分彩
			'3dfc' 		: { 'len' : 3, 'num' : '-', 'width' : 50 },//3d福彩
			'bjkl8' 	: { 'len' : 20, 'num' : '--' , 'classUl' : 'k8', 'width' : 20 },//北京快乐吧
			'60' 		: { 'len' : 3, 'num' : '' , 'classUl' : 'k3', 'width' : 51,'classSpan' : 'empty'}//江苏快三
		};
		if(_nodesHtml === ''){
			_key = sidebar_hover;
			_type = lottery_type[_key];
			if(typeof _type['classUl'] !== 'undefined'){//如果父容器有节点
				_node.addClass(_type['classUl']);	
			}

			for(i; i < _type['len'] ; i+=1){
				if(typeof _type['classLi'] !== 'undefined'){//如果li有class
					if(_key === 'xglhc'){//如果是六合彩
						if(i === _type['andIndex']){
							_html += '<li class="' + _type['andClass'] + '"><span></span></li>';
						}else{
							_html += '<li class="' + _type['classLi'] + '"><span>' + _type['num'] + '</span></li>';	
						}
					...
					if(typeof _type['classSpan'] !== 'undefined'){
						_html += '<li><span class="' + _type['classSpan'] + '">' + _type['num'] + '</span></li>';
					}else{
						_html += '<li><span>' + _type['num'] + '</span></li>';	
					}
					
				}
			}

			if(_key == 'bjkl8' || _key == 'pk10' || _key == 'jssm'){//单独处理北京快乐8和pk10
				_node.html(_html).css({'width': _type['width']*_type['len']/2 + "px"});
			}else{
				_node.html(_html).css({'width': _type['width']*_type['len'] + "px"});
			}
...
function gameKanJiangDataC(diffTime, actionNo){
	...
	if(diffTime<=0){
		if(game.type==20|| game.type==61) $('#num').removeClass("pk10_ul");
		codeplay('正在封单中');
	...
	$.ajax('/index.php/index/getLastKjData/'+type,{
		...
		success:function(data, textStatus, xhr){
			if(!data){
				...
				try{
					if(type==20) $('#num').addClass("pk10_ul");
					...
					playVoice(game.url+'/skin/sound/kai-jiang.wav', 'kai-jian-voice');
					gameFreshOrdered();
					reloadMemberInfo();
				}catch(err){
					setTimeout(loadKjData, 5000);
...
function safeBeforSetPwd(){
	if(!this.oldpassword.value){$.alert("请输入原密码");return false;}
	if(this.oldpassword.value.length<6){$.alert("原密码至少6位");return false;}
...
function safeBeforSetCBA(){
	if(!this.account.value){$.alert("银行帐号没有填写");return false;}
	if(!this.username.value){$.alert("银行开户名没有填写");return false;}
...
function indexSign(err, data){
	...
	if(err){
		document.getElementById('mainiframe').contentWindow.winjinAlert(err);
	}else{
		parent.reloadMemberInfo();
		document.getElementById('mainiframe').contentWindow.winjinAlert(data);
	}
} 
