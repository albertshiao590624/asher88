...
function gameKanJiangDataC(diffTime, actionNo){
	...
	if(diffTime<=0){
		if(game.type==20) $('#num').removeClass("pk10_ul");
		codeplay('正在封单中');
		...
		if(S){
			...
			$("#alert_close_button").val("(3) 确定");
			setTimeout(closekjtip, 3000);
		}
		S=false;
		KS=true;
		...
		if(S && h==0 && m==6 && s==0){
			playVoice(game.url+'/skin/sound/stop-time.wav', 'stop-time-voice');
		}
		if(h==0 && m==0 && s==0){
			loadKjData();
